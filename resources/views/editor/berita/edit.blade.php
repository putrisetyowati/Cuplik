...
<div class="w-full mt-6 pl-0 lg:pl-2">
    ...
    <div class="leading-loose">
        <form class="p-10 bg-white rounded shadow-xl" method="POST" action= "{{route('editor.news.update', $news->id)}}" enctype="multipart/form-data">
       @method('PUT')
        @csrf   
        <p class="text-xl text-gray-800 font-semibold pb-4">Edit Berita</p>

           

            <div class="mt-2">
                <label class="block text-sm text-gray-900" for="title">Title</label>
                <input class="w-full px-5 py-1 text-gray-700 bg-gray-200 rounded {{ $errors->has('title') ? 'is-invalid' : '' }}" id="title" name="title" type="text" required="" placeholder="Add Title" aria-label="Name" value="{{old('title', $news->title)}}">
                    @if($errors->has('title'))
            ...
            <div class="mt-2">
                ...
                <div class="relative w-full py-1 text-gray-700 rounded">
                    <select id="menu_id"
                        class="block p-2 w-full bg-gray-200 appearance-none focus:outline-none {{ $errors->has('menu_id') ? 'is-invalid' : '' }} " id="menu_id" required="" value="{{old('title', $news->id)}}"
                        name="id_menu">
            ...
            <div class="mt-2">
                ...
                <input class="w-full px-5 py-1 text-gray-700 bg-gray-200 rounded {{ $errors->has('source') ? 'is-invalid' : '' }} " id="source" name="source" type="text" required="" placeholder="Add Sumber Gambar" value="{{old('source', $news->source)}}">
            ...
            <div class="mt-2">
                ...
                <input class="w-full px-5 py-1 text-gray-700 bg-gray-200 rounded  {{ $errors->has('desc') ? 'is-invalid' : '' }}" id="desc" name="desc" type="text" required="" placeholder="Add Keterangan Gambar" value="{{old('desc', $news->desc)}}">
                @if($errors->has('desc'))
            ...
            <div class="mt-2 w-full">
                ...
                <img src=" {{asset('storage/img/news/'. $news->image)}}" alt="Image" class="avatar-img rounded-circle">
                <input class="form-control block w-full px-3 py-1.5 text-base font-normal {{ $errors->has('image') ? 'is-invalid' : '' }} text-gray-700 bg-white bg-clip-padding border border-solid border-gray-300 rounded transition ease-in-out m-0 focus:text-gray-700 focus:bg-white focus:border-blue-600 focus:outline-none" type="file" id="image" name="image" value="{{old('image', $news->image)}}">
                @if($errors->has('image'))
            ...
            <div class="col-span-5">  
                ...
                <trix-editor input="content"
                    class="relative outline-none border border-gray-400 rounded py-3 px-3 w-full bg-white shadow text-sm text-gray-700 focus:outline-none focus:shadow-outline {{ $errors->has('content') ? 'is-invalid' : '' }}" value="{{old('content', $news->content)}}">
                </trix-editor>
