@extends('editor.app')
@section('title','Tag Sub')
@section('content')
<div class="w-full mt-12">
    <div class="grid grid-cols-3 gap-4 mb-4">
        <div>
            <p class="text-xl pb-3 flex items-center">
            <i class="fas fa-list mr-3"></i> Tag-Sub
            </p>
        </div>
        <div class="col-span-2 place-self-end">
            <a href="{{route('editor.setting.tag-sub.create')}}" class="rounded bg-cyan-500 hover:bg-cyan-700 py-2 px-4 text-white transition duration-300 font-semibold">+ Tambah Data</a>
        </div>
    </div>
                        ...
                        <table class="min-w-full bg-white">
                            <thead class="bg-gray-800 text-white">
                                <tr>
                                    ...
                                    <th class="text-left py-3 px-4 uppercase font-semibold text-sm">Nama Tag</th>
                    ...
                    <tr>
                        ...
                        <td class="text-left py-3 px-4">{{$data->tag->name}}</td>
                        <td class="text-left py-3 px-4">{{$data->is_active}}</td> 
                        
                        
                        <td class="flex space-x-2 text-left py-3 px-4">
                    <a href="{{ url('editor/setting/tag-sub')}}/{{ $data->id }}/{{ 'edit' }}">
                        <div
                    ...
                    <form action="{{url('editor/setting/tag-sub')}}/{{$data->id}}" method="post">
                        @method('DELETE')
                        @csrf
                        <button type="submit"
